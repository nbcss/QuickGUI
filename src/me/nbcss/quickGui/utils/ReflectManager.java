...
public class ReflectManager {
    private static ReflectManager instance;

    public static ReflectManager get(){
        if(instance == null)
    ...
    public boolean ClassListEqual(Class<?>[] l1, Class<?>[] l2) {
	  ...
	  public Class<?> getCraftBukkitClass(String packageName, String craftClassName) throws ClassNotFoundException {
		   String version = Bukkit.getServer().getClass().getPackage().getName().replace(".", ",").split(",")[3] + ".";
		   StringBuffer name = new StringBuffer("org.bukkit.craftbukkit.");
		   name.append(version);
		   if(packageName != null)
			   name.append(packageName).append(".");
		   name.append(craftClassName);
		   Class<?> craftClass = Class.forName(name.toString());
		   return craftClass;
	  }
}
